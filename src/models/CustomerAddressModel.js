...
/**
 * Model get customer address by ID
 * @param {Number} id 
 * @returns Array
 */
const getCustomerAddressByIdModel = async (id) => {
  try {
    const customer = await (await db).execute('SELECT customer_address.id, customer_address.address, customer_address.customer_id, customer.customer_name FROM customer_address INNER JOIN customer ON customer_address.customer_id = customer.id WHERE customer_address.id = ?', [id]);
...
const updateCustomerAddressByIdModel = async (id, {customer_id, address}) => {
  try {
    const customer = await (await db).execute('UPDATE customer_address SET customer_id = ?, address = ? WHERE id = ?', [customer_id, address, id]);
